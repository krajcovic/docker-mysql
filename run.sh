...
# LOG="/var/lib/mysql/log/mysql-general.log"
...
StartMySQL ()
{
    /usr/bin/mysqld_safe --user=root ${EXTRA_OPTS} > /dev/null 2>&1 &
...
if [[ ! -d $VOLUME_HOME/mysql ]]; then
    ...
    mysql_install_db --explicit_defaults_for_timestamp || exit 1
...
if [ -n "${REPLICATION_MASTER}" ]; then
    ...
    if [ ! -f /replication_set.2 ]; then
        ...
        mysql -uroot -e "CREATE USER '${REPLICATION_USER}'@'127.0.0.1' IDENTIFIED BY '${REPLICATION_PASS}'"
        echo "=> GRANT REPLICATION SLAVE ON *.* TO '${REPLICATION_USER}'@'127.0.0.1'"
        mysql -uroot -e "GRANT REPLICATION SLAVE ON *.* TO '${REPLICATION_USER}'@'127.0.0.1'"
        mysql -uroot -e "--server-id=1"
        # echo "=> reset master"
